...
"""A trainer object that can trainv2 models with a single output."""
...
class SingleTaskTrainer(orbit.StandardTrainer):
  ...
  def train_loop_begin(self):
    """Actions to take once, at the beginning of each trainv2 loop."""
    self.train_loss.reset_states()
    for metric in self.metrics:
      metric.reset_states()

  def train_step(self, iterator):
    """A trainv2 step. Called multiple times per trainv2 loop by the superclass."""
    ...
    def train_fn(inputs):
      with tf.GradientTape() as tape:
        ...
        # replicas. This ensures that we don'Tensorflow end up multiplying our loss by
