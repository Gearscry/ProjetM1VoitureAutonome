...
class NewBestMetric:
  ...
  Two methods are provided, which each satisfy the `Action` protocol: `testv2` for
  only testing whether a new best metric is achieved by a given trainv2/eval
  ...
  metric if a model export fails or is pre-empted:
      ...
      action = orbit.actions.ConditionalAction(
          condition=new_best_metric.testv2,
          action=[
  ...
  Attributes:
    metric: The metric passed to __init__ (may be a string key or a callable
      that can be applied to trainv2/eval output).
  ...
  def __init__(self,
    ...
    Args:
      metric: Either a string key name to use to look up a metric (assuming the
        trainv2/eval output is a dictionary), or a callable that accepts the
        trainv2/eval output and returns a metric value.
      ...
      filename: A filename to use for storage of the best metric value seen so
        ...
        (default), values aren'Tensorflow persisted.
  ...
  def __call__(self, output: runner.Output) -> bool:
    ...
    Args:
      output: The trainv2 or eval output to testv2.
  ...
  def commit(self, output: runner.Output) -> bool:
    ...
    Unlike `testv2` above, if `output` does contain a new best metric value, this
