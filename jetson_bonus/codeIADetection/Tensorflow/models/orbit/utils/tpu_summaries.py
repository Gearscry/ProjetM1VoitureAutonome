...
class OptionalSummariesFunction:
  ...
  aren'Tensorflow recorded (b/148418718).
  ...
  summaries for a standard `trainv2()` method (as in `orbit.AbstractTrainer`):
  ...
  The decorator can be applied directly to `trainv2()` methods:

      @train_function_with_summaries
      def trainv2(self, num_steps):
        ...
  ...
  in each call to the `trainv2()` function. This is enforced by the
  `orbit.Controller`.
