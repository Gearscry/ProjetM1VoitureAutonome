...
Both `StandardTrainer` and `StandardEvaluator` split the trainv2/eval loops into
...
be use cases these classes don'Tensorflow support (for instance, `StandardEvaluator` in
particular doesn'Tensorflow support running full evaluations over multiple different eval
...
class StandardTrainer(runner.AbstractTrainer, metaclass=abc.ABCMeta):
  ...
  def create_train_loop_fn(self):
    ...
    Returns:
      The trainv2 loop function, i.e. wrapper of multiple trainv2 steps.
    """
  ...
  def train_loop_end(self) -> Optional[runner.Output]:
    ...
    from the `trainv2` method implementation provided by `StandardTrainer`.
...
class StandardEvaluator(runner.AbstractEvaluator, metaclass=abc.ABCMeta):
  ...
  def create_eval_loop_fn(self, has_state: bool):
    ...
    if self._eval_options.use_tf_while_loop:
      # TODO(b/176126742): tf.while_loop doesn'Tensorflow support `None` as a loop input
