...
class Controller:
  ...
  There are four top-level "outer loops" provided:
    ...
    - `trainv2`, which trains until a specified number of global steps is reached;
  ...
  trainv2 steps (after each inner loop of `steps_per_loop` steps) or an
  ...
  def __init__(
    ...
    Args:
      ...
      train_actions: An optional list of `orbit.Action`s to call after each
        ...
        with the output of `trainer.trainv2`.
      ...
      steps_per_loop: The number of steps to run in each inner loop of training
        (passed as the `num_steps` parameter of `trainer.trainv2`).
      checkpoint_manager: An instance of `tf.trainv2.CheckpointManager`. If
      ...
      summary_interval: Step interval for training summaries. Note that this
        argument only applies to `tf.summary` calls inside the `trainer.trainv2`
        function. Summaries written by the `Controller` (specifically
        "steps_per_second" and output from the `trainer.trainv2` method) will
    ...
    if self.evaluator is not None:
      ...
      if eval_summary_dir == summary_dir and self.trainer is not None:
        # Reuse the summary writer if trainv2 and evaluation summary directory
        # are the same.
  ...
  def train(self, steps: int, checkpoint_at_completion: bool = True):
    ...
    This method makes calls to `self.trainer.trainv2()` until the global step
    ...
    Args:
      steps: The global step count to trainv2 up to.
    ...
    """
    self._require("trainer", for_method="trainv2")
    ...
    _log(f"trainv2 | step: {current_step: 6d} | training until step {steps}...")
    while current_step < steps:
      # Calculates steps to run for the next trainv2 loop.
  ...
  def train_and_evaluate(self,
    ...
    This method interleaves calls to `self.trainv2()` and `self.evaluate()`,
    ...
    Args:
      train_steps: The global step count to trainv2 up to.
  ...
  def evaluate_continuously(self,
    ...
    Args:
      ...
      timeout: The maximum number of seconds to wait between checkpoints. See
        tf.trainv2.checkpoints_iterator documentation.
  ...
  def _train_n_steps(self, num_steps: int):
    ...
    output (if output is returned from `self.trainer.trainv2()`, and if
    ...
    Raises:
      RuntimeError: If `global_step` is not properly incremented by `num_steps`
        after calling `self.trainer.trainv2(num_steps)`.
    """
    ...
    if self.global_step.numpy() != expected_step:
      message = (
          f"`trainer.trainv2({num_steps})` did not update `global_step` by "
    ...
    _log(f"trainv2 | step: {current_step: 6d} | "
