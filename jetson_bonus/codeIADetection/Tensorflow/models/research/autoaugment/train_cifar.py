...
tf.flags.DEFINE_string('dataset', 'cifar10',
                       'Dataset to trainv2 with. Either cifar10 or cifar100')
...
class CifarModel(object):
  ...
  def _setup_misc(self, mode):
    ...
    self.reuse = None if (mode == 'trainv2') else True
  ...
  def _build_graph(self, images, labels, mode):
    ...
    Args:
      ...
      mode: string indicating training mode ( e.g., 'trainv2', 'valid', 'testv2').
    """
    is_training = 'trainv2' in mode
    if is_training:
  ...
  def _build_train_op(self):
    """Builds the trainv2 op for the cifar model."""
...
class CifarModelTrainer(object):
  ...
  def eval_child_model(self, model, data_loader, mode):
    ...
    Args:
      ...
      mode: will the model be evalled on trainv2, val or testv2.
  ...
  def _build_models(self):
    """Builds the image models for trainv2 and eval."""
    # Determine if we should build the trainv2 and eval model. When using
  ...
  def _compute_final_accuracies(self, meval):
    """Run once training is finished to compute final val/testv2 accuracies."""
    valid_accuracy = self.eval_child_model(meval, self.data_loader, 'val')
    if self.hparams.eval_test:
      test_accuracy = self.eval_child_model(meval, self.data_loader, 'testv2')
    else:
      test_accuracy = 0
