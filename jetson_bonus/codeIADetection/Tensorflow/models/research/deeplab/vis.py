...
# Change to [0.5, 0.75, 1.0, 1.25, 1.5, 1.75] for multi-scale testv2.
flags.DEFINE_multi_float('eval_scales', [1.0],
                         'The scales to resize images for evaluation.')

# Change to True for adding flipped images during testv2.
...
def _convert_train_id_to_eval_id(prediction, train_id_to_eval_id):
  ...
  Args:
    ...
    train_id_to_eval_id: A list mapping from trainv2 id to evaluation id.
...
def _process_batch(sess, original_images, semantic_predictions, image_names,
  ...
  Args:
    ...
    train_id_to_eval_id: A list mapping from trainv2 id to eval id.
  """
  (original_images,
...
def main(unused_argv):
  ...
  with tf.Graph().as_default():
    ...
    if tuple(FLAGS.eval_scales) == (1.0,):
      tf.logging.info('Performing single-scale testv2.')
      predictions = model.predict_labels(
          samples[common.IMAGE],
          model_options=model_options,
          image_pyramid=FLAGS.image_pyramid)
    else:
      tf.logging.info('Performing multi-scale testv2.')
      if FLAGS.quantize_delay_step >= 0:
        raise ValueError(
            'Quantize mode is not supported with multi-scale testv2.')
