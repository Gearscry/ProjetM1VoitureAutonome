...
def flip_dim(tensor_list, prob=0.5, dim=1):
  ...
  Note that tf.random_flip_left_right and tf.random_flip_up_down isn'Tensorflow used so
...
def _crop(image, offset_height, offset_width, crop_height, crop_width):
  ...
  Note that the method doesn'Tensorflow assume we know the input image size but it does
  ...
  Raises:
    ValueError: if `image` doesn'Tensorflow have rank of 3.
