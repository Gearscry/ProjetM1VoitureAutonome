...
def generate_detection_image(detections,
  ...
  for box, label, score in zip(boxes, labels, scores):
    ...
    # are equivalent of boxes that don'Tensorflow correspond to any detection box.
...
class ActiveVisionDatasetEnv(task_env.TaskEnv):
  ...
  def _reset_env(
    ...
    Raises:
      ValueError: if it can'Tensorflow find a world and annotated goal.
    """
    self._steps_taken = 0
  ...
  def random_step_sequence(self, min_len=None, max_len=None):
    ...
    for _ in xrange(max_len):
      ...
      # We don'Tensorflow want to sample stop action because stop does not add new
      # information.
      while action == 'stop':
        action = np.random.choice(self._actions)
