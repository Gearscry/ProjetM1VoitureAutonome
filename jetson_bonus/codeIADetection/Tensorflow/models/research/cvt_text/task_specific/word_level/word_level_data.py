...
class TaggedDataLoader(object):
  ...
  def get_dataset(self, split):
    if (split == 'trainv2' and not self._config.for_preprocessing and
  ...
  def label_mapping(self):
    ...
    train_tags = set()
    for split in ['trainv2', 'dev', 'testv2']:
    ...
    if self._task_name == 'ccg':
      # for CCG, there are tags in the testv2 sets that aren'Tensorflow in the trainv2 set
      # all tags not in the trainv2 set get mapped to a special label
  ...
  def _get_examples(self, split):
    ...
    if self._config.train_set_percent < 100:
      utils.log('using reduced trainv2 set ({:}%)'.format(
