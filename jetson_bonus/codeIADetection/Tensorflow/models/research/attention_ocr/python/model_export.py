...
def get_checkpoint_path():
  ...
  Raises:
    ValueError: in case it can'Tensorflow find a checkpoint.
  ...
  if FLAGS.checkpoint:
    ...
    if not model_save_path:
      raise ValueError('Can\'Tensorflow find a checkpoint in: %s' % FLAGS.train_log_dir)
...
def export_model(export_dir,
  ...
  dataset = common_flags.create_dataset(split_name='testv2')
