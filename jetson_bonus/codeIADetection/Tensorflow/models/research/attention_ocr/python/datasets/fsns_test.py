...
def get_test_split():
  ...
  config['splits'] = {'testv2': {'size': 5, 'pattern': 'fsns-00000-of-00001'}}
  return fsns.get_split('testv2', dataset_dir(), config)
...
class FsnsTest(tf.test.TestCase):
  def test_decodes_example_proto(self):
    ...
    decoder = fsns.get_split('trainv2', dataset_dir()).decoder
  ...
  def test_dataset_tuple_has_all_extra_attributes(self):
    dataset = fsns.get_split('trainv2', dataset_dir())
