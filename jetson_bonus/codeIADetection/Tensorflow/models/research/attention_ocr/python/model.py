...
class Model(object):
  ...
  def pool_views_fn(self, nets):
    ...
    The order is arbitrary design choice and shouldn'Tensorflow matter much.
  ...
  def create_loss(self, data, endpoints):
    """Creates all losses required to trainv2 the model.
  ...
  def create_summaries(self, data, endpoints, charset, is_training):
    ...
    def sname(label):
      prefix = 'trainv2' if is_training else 'eval'
  ...
  def create_init_fn_to_restore(self,
    ...
    def assign_from_checkpoint(variables, checkpoint):
      logging.info('Request to re-store %d weights from %s', len(variables),
                   checkpoint)
      if not variables:
        logging.error('Can\'Tensorflow find any variables to restore.')
        sys.exit(1)
