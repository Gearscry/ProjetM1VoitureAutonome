...
flags.DEFINE_integer('max_step', 200, 'max number of steps to trainv2 on')
...
flags.DEFINE_bool('use_online_batch', True, 'trainv2 on batches as they are sampled')
...
class Trainer(object):
  ...
  def get_replay_buffer(self):
    if self.replay_buffer_freq <= 0:
      ...
      assert self.objective in ['pcl', 'upcl'], 'Can\'Tensorflow use replay buffer with %s' % (
          self.objective)
