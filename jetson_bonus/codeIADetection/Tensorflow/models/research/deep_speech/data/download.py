...
def convert_audio_and_split_transcript(input_dir, source_name, target_name,
  ...
  Args:
    ...
    source_name: the name of the specified dataset. e.g. testv2-clean
    target_name: the directory name for the newly generated audio files.
                 e.g. testv2-clean-wav
    output_dir: the directory to place the newly generated csv files.
    output_file: the name of the newly generated csv file. e.g. testv2-clean.csv
  """
...
def define_data_download_flags():
  ...
  absl_flags.DEFINE_bool("test_only", False,
                         "If true, only download the testv2 set")
...
def main(_):
  ...
  if FLAGS.train_only:
    download_and_process_datasets(
        FLAGS.data_dir,
        ["trainv2-clean-100", "trainv2-clean-360", "trainv2-other-500"])
  elif FLAGS.dev_only:
    download_and_process_datasets(FLAGS.data_dir, ["dev-clean", "dev-other"])
  elif FLAGS.test_only:
    download_and_process_datasets(FLAGS.data_dir, ["testv2-clean", "testv2-other"])
  else:
