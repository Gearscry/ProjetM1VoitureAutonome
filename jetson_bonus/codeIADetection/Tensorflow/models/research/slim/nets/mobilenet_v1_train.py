...
# ==============================================================================
"""Build and trainv2 mobilenet_v1 with options for quantization."""
...
def get_quant_delay():
  if FLAGS.fine_tune_checkpoint:
    ...
    # We need to wait for the model to trainv2 a bit before we quantize if we are
...
def imagenet_input(is_training):
  ...
  Args:
     is_training: bool specifying if trainv2 or validation dataset is needed.
  Returns:
     A batch of images and labels.
  """
  if is_training:
    dataset = dataset_factory.get_dataset('imagenet', 'trainv2',
                                          FLAGS.dataset_dir)
  else:
...
def build_model():
  """Builds graph for model to trainv2 with rewrites for quantization.
