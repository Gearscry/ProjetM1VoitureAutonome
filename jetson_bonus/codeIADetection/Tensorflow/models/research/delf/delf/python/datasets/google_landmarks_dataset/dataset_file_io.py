...
def ReadSolution(file_path, task):
  ...
  Returns:
    public_solution: Dict mapping testv2 image ID to list of ground-truth IDs, for
      the Public subset of testv2 images. If `task` == 'recognition', the IDs are
    ...
    private_solution: Same as `public_solution`, but for the private subset of
      testv2 images.
    ignored_ids: List of testv2 images that are ignored in scoring.
...
def ReadPredictions(file_path, public_ids, private_ids, ignored_ids, task):
  ...
  Args:
    ...
    public_ids: Set (or list) of testv2 image IDs in Public subset of testv2 images.
    private_ids: Same as `public_ids`, but for the private subset of testv2
      images.
    ignored_ids: Set (or list) of testv2 image IDs that are ignored in scoring and
      are associated to no ground-truth.
    task: Type of challenge task. Supported values: 'recognition', 'retrieval'.

  Returns:
    public_predictions: Dict mapping testv2 image ID to prediction, for the Public
      subset of testv2 images. If `task` == 'recognition', the prediction is a
    ...
    private_predictions: Same as `public_predictions`, but for the private
      subset of testv2 images.

  Raises:
    ValueError:
      - If testv2 image ID is unrecognized/repeated;
        ...
        if task == RECOGNITION_TASK_ID:
          if len(prediction_split) != 2:
            raise ValueError('Prediction is malformed: there should only be 2 '
                             'elements in second column, but found %d for testv2 '
