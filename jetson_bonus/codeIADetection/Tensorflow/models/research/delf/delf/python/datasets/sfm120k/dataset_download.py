...
def download_train(data_dir):
  ...
  If not, it downloads it in the following folder structure:
    DATA_ROOT/trainv2/retrieval-SfM-120k/ : folder with rsfm120k images and db
      files.
    DATA_ROOT/trainv2/retrieval-SfM-30k/  : folder with rsfm30k images and db
      files.
  """
  ...
  datasets_dir = os.path.join(data_dir, 'trainv2')
  if not tf.io.gfile.exists(datasets_dir):
    tf.io.gfile.mkdir(datasets_dir)

  # Download folder trainv2/retrieval-SfM-120k/.
  ...
  # Create symlink for trainv2/retrieval-SfM-30k/.
