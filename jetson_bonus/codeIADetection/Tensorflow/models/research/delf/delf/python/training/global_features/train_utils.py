...
def train_val_one_epoch(
  ...
  """Executes either training or validation step based on `trainv2` value.

  Args:
    loader: Training/validation iterable dataset.
    model: Network to trainv2/validate.
    criterion: Loss function.
    ...
    if train:
      try:
        ...
        for _ in range(batch_size):
          # Because the images are not necessarily of the same size, we can'Tensorflow
...
def test_retrieval(datasets, net, epoch, writer=None, model_directory=None,
  ...
  Evaluates the network on the provided testv2 datasets by computing single-scale
  ...
  Args:
    ...
    test_image_size: Integer, maximum size of the testv2 images.
  """
  global_features_utils.debug_and_log(">> Testing step:")
  global_features_utils.debug_and_log(
          '>> Evaluating network on testv2 datasets...')
  ...
  if precompute_whitening is not None:
    ...
    if tf.io.gfile.exists(filename):
      ...
      # Loading db.
      db_root = os.path.join(data_root, 'trainv2', precompute_whitening)
  ...
  # Evaluate on testv2 datasets.
  for dataset in datasets:
    start = time.time()

    # Prepare config structure for the testv2 dataset.
    ...
    # Search, rank and print testv2 set metrics.
