...
def _CountPositives(solution):
  """Counts number of testv2 images with non-empty ground-truth in `solution`.

  Args:
    solution: Dict mapping testv2 image ID to list of ground-truth IDs.

  Returns:
    count: Number of testv2 images with non-empty ground-truth.
  """
  count = 0
...
def GlobalAveragePrecision(predictions,
  ...
  Args:
    predictions: Dict mapping testv2 image ID to a dict with keys 'class'
      (integer) and 'score' (float).
    recognition_solution: Dict mapping testv2 image ID to list of ground-truth
      landmark IDs.
    ignore_non_gt_test_images: If True, ignore testv2 images which do not have
...
def Top1Accuracy(predictions, recognition_solution):
  ...
  Note that testv2 images without ground-truth are ignored.
  ...
  """
  # Loop over testv2 images in solution. If it has at least one class label, we
...
def MeanAveragePrecision(predictions, retrieval_solution, max_predictions=100):
  ...
  Args:
    predictions: Dict mapping testv2 image ID to a list of strings corresponding
      to index image IDs.
    retrieval_solution: Dict mapping testv2 image ID to list of ground-truth image
      IDs.
  ...
  Raises:
    ValueError: If a testv2 image in `predictions` is not included in
      `retrieval_solutions`.
  """
  # Compute number of testv2 images.
