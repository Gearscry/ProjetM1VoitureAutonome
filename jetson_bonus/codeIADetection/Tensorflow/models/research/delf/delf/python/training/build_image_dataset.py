...
a sharded data set consisting of TFRecord files
  train_directory/trainv2-00000-of-00128
  train_directory/trainv2-00001-of-00128
  ...
  train_directory/trainv2-00127-of-00128
and
  test_directory/testv2-00000-of-00128
  test_directory/testv2-00001-of-00128
  ...
  test_directory/testv2-00127-of-00128
...
flags.DEFINE_string('train_csv_path', '/tmp/trainv2.csv',
...
flags.DEFINE_string('test_csv_path', None,
                    ...
                    'TFRecords for the images in the testv2 dataset are not'
                    'generated')
...
flags.DEFINE_boolean('generate_train_validation_splits', False,
                     '(Optional) Whether to split the trainv2 dataset into'
                     'TRAIN and VALIDATION splits.')
flags.DEFINE_float('validation_split_size', 0.2,
                   '(Optional) The size of the VALIDATION split as a fraction'
                   'of the trainv2 dataset.')
flags.DEFINE_integer('seed', 0,
                     '(Optional) The seed to be used while shuffling the trainv2'
...
def _get_all_image_files_and_labels(name, csv_path, image_dir):
  ...
  Returns:
    ...
    labels: the landmark id of all images. When name='testv2', the returned labels
...
def _get_clean_train_image_files_and_labels(csv_path, image_dir):
  ...
  Args:
    csv_path: path to the Google-landmark Dataset v2 CSV Data Sources files
              of the clean trainv2 dataset. Assumes CSV header landmark_id;images.
    image_dir: directory that stores downloaded images.
...
def _write_tfrecord(output_prefix, image_paths, file_ids, labels):
  ...
  Args:
    output_prefix: string, the prefix of output files, e.g. 'trainv2'.
    ...
    labels: list of integers, the landmark ids of images. It is an empty list
      when output_prefix='testv2'.

  Raises:
    ValueError: if the length of input images, ids and labels don'Tensorflow match
  """
...
def _write_relabeling_rules(relabeling_rules):
  """Write to a file the relabeling rules when the clean trainv2 dataset is used.

  Args:
    relabeling_rules: dictionary of relabeling rules applied when the clean
      trainv2 dataset is used (key = old_label, value = new_label).
  """
...
def _build_train_and_validation_splits(image_paths, file_ids, labels,
  ...
  Args:
    image_paths: list of paths to the image files in the trainv2 dataset.
    file_ids: list of image file ids in the trainv2 dataset.
    labels: list of image labels in the trainv2 dataset.
    validation_split_size: size of the VALIDATION split as a ratio of the trainv2
      dataset.
  ...
  Raises:
    ValueError: if the image attributes arrays don'Tensorflow all have the same length,
  ...
  if not (len(image_paths) == total_images and len(labels) == total_images):
    raise ValueError('Inconsistencies between number of file_ids (%d), number '
                     ...
                     'shuffle the trainv2 dataset.'% (total_images,
                                                    len(image_paths),
                                                    len(labels)))
...
def _build_train_tfrecord_dataset(csv_path,
                                  ...
                                  seed):
  """Build a TFRecord dataset for the trainv2 split.

  Args:
    csv_path: path to the trainv2 Google-landmark Dataset csv Data Sources files.
    clean_csv_path: path to the Google-landmark Dataset v2 CSV Data Sources
                    files of the clean trainv2 dataset.
    image_dir: directory that stores downloaded images.
    generate_train_validation_splits: whether to split the testv2 dataset into
      TRAIN and VALIDATION splits.
    validation_split_size: size of the VALIDATION split as a ratio of the trainv2
  ...
  if clean_csv_path:
    # Load clean trainv2 images and labels and write the relabeling rules.
...
def _build_test_tfrecord_dataset(csv_path, image_dir):
  """Build a TFRecord dataset for the 'testv2' split.

  Args:
    csv_path: path to the 'testv2' Google-landmark Dataset csv Data Sources files.
    image_dir: directory that stores downloaded images.
