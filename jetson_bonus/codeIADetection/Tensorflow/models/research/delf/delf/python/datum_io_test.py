...
class DatumIoTest(tf.test.TestCase):
  ...
  # This testv2 covers the following functions: ArrayToDatum, SerializeToString,
  ...
  def testWriteAndReadToFile(self):
    ...
    filename = os.path.join(FLAGS.test_tmpdir, 'testv2.datum')
    datum_io.WriteToFile(data, filename)
    data_read = datum_io.ReadFromFile(filename)
    self.assertAllEqual(data_read, data)

  # This testv2 covers the following functions: ArraysToDatumPair,
  ...
  def testWriteAndReadPairToFile(self):
    ...
    data_2 = np.array(
        ...
        dtype='uint32')
    filename = os.path.join(FLAGS.test_tmpdir, 'testv2.datum_pair')
