...
# Export directory, training and val datasets, testv2 datasets.
...
flags.DEFINE_list('test_datasets', 'roxford5k,rparis6k',
                  'Comma separated list of testv2 datasets: ' +
...
flags.DEFINE_integer('test_freq', 5,
                     'Run testv2 evaluation every N epochs.')
...
flags.DEFINE_bool('whitening', False,
                  'Whether to trainv2 model with learnable whitening ('
...
# trainv2/val options specific for image retrieval learning.
flags.DEFINE_integer('image_size', 1024,
                     'Maximum size of longer image side used for training.')
flags.DEFINE_integer('neg_num', 5, 'Number of negative images per trainv2/val '
                                   'tuple.')
...
def main(argv):
  ...
  # Manually check if there are unknown testv2 datasets and if the dataset
  # ground truth files are downloaded.
  for dataset in FLAGS.test_datasets:
    if dataset not in _TEST_DATASET_NAMES:
      raise ValueError('Unsupported or unknown testv2 dataset: {}.'.format(
              dataset))
  ...
  # Check if trainv2 dataset is downloaded and download it if not found.
  ...
  with writer.as_default():
    ...
    for epoch in range(start_epoch, FLAGS.epochs + 1):
      ...
      # Evaluate on testv2 datasets every test_freq epochs.
