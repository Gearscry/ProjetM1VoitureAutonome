...
# ==============================================================================
"""Script to trainv2 langid model.

The script builds language detection from wikipedia dataset,
builds SGNN model to trainv2 an on-device model to
...
def dataset_fn(batch_size, is_training, split, try_gcs, max_input_len):
  """Creates dataset to trainv2 and evaluate.
...
def train_and_evaluate():
  ...
  model.fit(
      dataset_fn(FLAGS.batch_size, True, 'trainv2[:10%]', True, 100),
      epochs=FLAGS.epochs,
      steps_per_epoch=1000,
      validation_steps=100,
      validation_data=dataset_fn(FLAGS.batch_size, False, 'trainv2[10:11%]', True,
                                 100),
  )
