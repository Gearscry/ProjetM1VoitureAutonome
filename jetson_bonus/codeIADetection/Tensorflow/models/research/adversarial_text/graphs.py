...
class VatxtModel(object):
  ...
  def classifier_graph(self):
    ...
    * Caches tensors: `cl_embedded`, `cl_logits`, `cl_loss`
    ...
    Returns:
      loss: scalar float.
    """
    inputs = _inputs('trainv2', pretrain=False)
  ...
  def language_model_graph(self, compute_loss=True):
    ...
    * Caches tensors: `lm_embedded`
    ...
    Returns:
      loss: scalar float.
    """
    inputs = _inputs('trainv2', pretrain=True)
  ...
  def eval_graph(self, dataset='testv2'):
    """Constructs classifier evaluation graph.

    Args:
      dataset: the labeled dataset to evaluate, {'trainv2', 'testv2', 'valid'}.
  ...
  def adversarial_loss(self):
    ...
    def virtual_adversarial_loss():
      ...
      Uses lm_inputs and constructs the language model graph if it hasn'Tensorflow yet
      been constructed.
...
class VatxtBidirModel(VatxtModel):
  ...
  def classifier_graph(self):
    ...
    * Caches tensors: `cl_embedded` (tuple of forward and reverse), `cl_logits`,
    ...
    Returns:
      loss: scalar float.
    """
    inputs = _inputs('trainv2', pretrain=False, bidir=True)
    self.cl_inputs = inputs
    f_inputs, _ = inputs
  ...
  def language_model_graph(self, compute_loss=True):
    ...
    * Caches tensors: `lm_embedded`, `lm_embedded_reverse`
    ...
    """
    inputs = _inputs('trainv2', pretrain=True, bidir=True)
    self.lm_inputs = inputs
...
def _inputs(dataset='trainv2', pretrain=False, bidir=False):
