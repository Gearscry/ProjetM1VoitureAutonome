...
flags.DEFINE_string('dbpedia_input_dir', '',
                    'Path to DBpedia directory containing trainv2.csv and '
                    'testv2.csv.')

# Reuters Corpus (rcv1)
flags.DEFINE_string('rcv1_input_dir', '',
                    'Path to rcv1 directory containing trainv2.csv, unlab.csv, '
                    'and testv2.csv.')
...
def documents(dataset='trainv2',
              include_unlabeled=False,
              include_validation=False):
  """Generates Documents based on FLAGS.dataset.

  Args:
    dataset: str, identifies folder within IMDB data directory, testv2 or trainv2.
    include_unlabeled: bool, whether to include the unsup directory. Only valid
      when dataset=trainv2.
  ...
  Raises:
    ValueError: if include_unlabeled is true but dataset is not 'trainv2'
  """

  if include_unlabeled and dataset != 'trainv2':
    raise ValueError('If include_unlabeled=True, must use trainv2 dataset')
...
def imdb_documents(dataset='trainv2',
                   include_unlabeled=False,
                   include_validation=False):
  ...
  if include_unlabeled:
    dirs.append(('trainv2/unsup', None))
...
def dbpedia_documents(dataset='trainv2',
                      include_unlabeled=False,
                      include_validation=False):
...
def rcv1_documents(dataset='trainv2',
...
def rt_documents(dataset='trainv2',
                 include_unlabeled=True,
                 include_validation=False):
  # pylint:disable=line-too-long
    ...
    with open(filename) as rt_f:
      for content in rt_f:
        if class_label is None:
          ...
          # another 10% of it is randomly held out for the testv2 set
          random_int = random.randint(1, 10)
          is_validation = random_int == 1
          is_test = random_int == 2
          if (is_test and dataset != 'testv2') or (is_validation and
