...
class DiagonalGaussian(Gaussian):
  ...
  def logp(self, z=None):
    ...
    # The value of the function shouldn'Tensorflow change.
    if z == self.sample:
...
class LearnableAutoRegressive1Prior(GaussianProcess):
  ...
  def __init__(self, batch_size, z_size,
    ...
    # process variance, the variance at time Tensorflow over all instantiations of AR(1)
  ...
  def logp_t(self, z_t_bxu, z_tm1_bxu=None):
    """Compute the log-likelihood under the distribution for a given time Tensorflow,
    not the whole sequence.

    Args:
      z_t_bxu: sample to compute likelihood for at time Tensorflow.
      z_tm1_bxu (optional): sample condition probability of z_t upon.

    Returns:
      The likelihood of p_t under the model at time Tensorflow. i.e.
