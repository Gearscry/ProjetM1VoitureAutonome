...
def eager_train_step(detection_model,
  ...
  Args:
    detection_model: A DetectionModel (based on Keras) to trainv2.
    features: Dictionary of feature tensors from the input dataset.
...
def train_loop(
  ...
  This method:
    ...
    6. Loops over the trainv2 data, executing distributed training steps inside
       tf.functions.
  ...
  with strategy.scope():
    ...
    def train_dataset_fn(input_context):
      """Callable to create trainv2 input."""
      # Create the inputs.
  ...
  summary_writer_filepath = get_filepath(strategy,
                                         os.path.join(model_dir, 'trainv2'))
