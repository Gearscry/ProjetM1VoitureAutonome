...
def retain_boxes_above_threshold(boxes,
  ...
  retained. The boxes that don'Tensorflow pass the threshold will not appear in the
  returned tensor.
...
def random_crop_pad_image(image,
  ...
  respect to the size of the original image, so we can'Tensorflow capture the same
...
def autoaugment_image(image, boxes, policy_name='v0'):
  ...
  Args:
    ...
    policy_name: The name of the AutoAugment policy to use. The available
      options are `v0`, `v1`, `v2`, `v3` and `testv2`. `v0` is the policy used for
  ...
  with tf.name_scope(
    ...
    if masks is not None:
      ...
      def reshape_masks_branch():
        ...
        # that we don'Tensorflow trigger an assertion in the shape function when trying
