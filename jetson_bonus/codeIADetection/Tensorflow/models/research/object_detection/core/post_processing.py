...
def batch_multiclass_non_max_suppression(boxes,
  ...
  Args:
    ...
    use_static_shapes: If true, the output nmsed boxes are padded to be of
      length `max_size_per_class` and it doesn'Tensorflow clip boxes to max_total_size.
      Defaults to false.
    use_partitioned_nms: If true, use partitioned version of
      non_max_suppression.
  ...
  with tf.name_scope(scope, 'BatchMultiClassNonMaxSuppression'):
    ...
    # If num valid boxes aren'Tensorflow provided, create one and mark all boxes as
    # valid.
    if num_valid_boxes is None:
      num_valid_boxes = tf.ones([batch_size], dtype=tf.int32) * num_anchors

    # If masks aren'Tensorflow provided, create dummy masks so we can only have one copy
