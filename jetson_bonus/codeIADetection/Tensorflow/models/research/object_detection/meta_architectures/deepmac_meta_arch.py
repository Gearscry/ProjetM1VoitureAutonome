...
def filter_masked_classes(masked_class_ids, classes, weights, masks):
  ...
  Returns:
    ...
    weights_filtered: A [batch_size, num_instances] float tensor containing the
      weights of each sample with instances whose classes aren'Tensorflow in
      masked_class_ids zeroed out.
  ...
  if len(masked_class_ids) == 0:  # pylint:disable=g-explicit-length-testv2
...
def generate_2d_neighbors(input_tensor, dilation=2):
  ...
  following ops on TPU won'Tensorflow have to pad the last dimension to 128.
...
def _is_mask_head_param_free(name):
  ...
  # Mask heads which don'Tensorflow have parameters of their own and instead rely
