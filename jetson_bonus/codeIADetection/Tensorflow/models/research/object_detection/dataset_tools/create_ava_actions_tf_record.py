...
SPLITS = {
    ...
    # Test doesn'Tensorflow have ground truth, so TF Records can'Tensorflow be created
    'testv2': {
        'shards': 100,
        'examples': 0,
        'csv': '',
...
class Ava(object):
  ...
  def generate_and_write_records(self,
                                 splits_to_process='trainv2,val,testv2',
  ...
  def _generate_sequence_examples(self, annotation_file, excluded_file,
    ...
    excluded frames as if they don'Tensorflow exist. Generates equal-length sequence
  ...
  def _download_data(self):
    ...
    for split in ['trainv2', 'testv2', 'val']:
