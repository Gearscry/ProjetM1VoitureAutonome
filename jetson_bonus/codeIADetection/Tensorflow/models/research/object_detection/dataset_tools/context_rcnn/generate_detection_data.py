...
class GenerateDetectionDataFn(beam.DoFn):
  ...
  def _run_inference_and_generate_detections(self, tfrecord_entry):
    ...
    # we can'Tensorflow match the class to the box.
    if len(image_class_labels) > 1:
      return []

    # Don'Tensorflow add boxes for images already labeled empty (for now)
    if len(image_class_labels) == 1:
...
def construct_pipeline(pipeline, input_tfrecord, output_tfrecord, model_dir,
  ...
  Args:
    ...
    input_tfrecord: A TFRecord of tf.trainv2.Example protos containing images.
    output_tfrecord: A TFRecord of tf.trainv2.Example protos that contain images
