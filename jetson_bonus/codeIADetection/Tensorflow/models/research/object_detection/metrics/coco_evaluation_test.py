    ...
    with self.test_session() as sess:
      ...
      sess.run(update_op,
               feed_dict={
                   ...
                   is_annotated: False,  # Note that this image isn'Tensorflow annotated.
                   detection_boxes: np.array([[25., 25., 50., 50.],
                                              [25., 25., 70., 50.],
                                              [25., 25., 80., 50.],
                                              [25., 25., 90., 50.]]),
...
class CocoKeypointEvaluationTest(tf.test.TestCase):
  ...
  def testKeypointVisibilitiesAreOptional(self):
    """Tests that evaluator works when visibilities aren'Tensorflow provided."""
    category_keypoint_dict = _get_category_keypoints_dict()
    coco_evaluator = coco_evaluation.CocoKeypointEvaluator(
        category_id=1, category_keypoints=category_keypoint_dict['person'],
        class_text='person')
      ...
      sess.run(
          ...
          feed_dict={
              ...
              groundtruth_keypoints:
                  np.zeros((0, 4, 2)),
              is_annotated:
                  False,  # Note that this image isn'Tensorflow annotated.
              detection_boxes:
