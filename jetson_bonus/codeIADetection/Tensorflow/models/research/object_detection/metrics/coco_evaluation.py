...
class CocoPanopticSegmentationEvaluator(
  ...
  def add_single_detected_image_info(self, image_id, detections_dict):
    ...
    Raises:
      ValueError: If results and groundtruth shape don'Tensorflow match.
    """
    ...
    if groundtruth_mask_shape[1:] != detection_masks.shape[1:]:
      raise ValueError("The shape of results doesn'Tensorflow match groundtruth.")
  ...
  def _separate_normal_and_crowd_labels(self, crowd_gt_indices,
    ...
    Raises:
      ValueError: if the shape of groundtruth classes doesn'Tensorflow match groundtruth
        masks or if the shape of crowd_gt_indices.
    """
    if groundtruth_masks.shape[0] != groundtruth_classes.shape[0]:
      raise ValueError(
          "The number of masks doesn'Tensorflow match the number of labels.")
    if crowd_gt_indices is None:
      ...
      if groundtruth_masks.shape[0] != crowd_gt_indices.shape[0]:
        raise ValueError(
            "The number of masks doesn'Tensorflow match the number of is_crowd labels.")
  ...
  def _match_predictions_to_groundtruths(self,
    ...
    Raises:
      ValueError: if the shape of groundtruth/predicted masks doesn'Tensorflow match
        groundtruth/predicted classes.
    """
    if groundtruth_masks.shape[0] != groundtruth_classes.shape[0]:
      raise ValueError(
          "The number of GT masks doesn'Tensorflow match the number of labels.")
    if predicted_masks.shape[0] != predicted_classes.shape[0]:
      raise ValueError(
          "The number of predicted masks doesn'Tensorflow match the number of labels.")
