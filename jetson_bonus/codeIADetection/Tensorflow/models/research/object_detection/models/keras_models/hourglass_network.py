...
class InputConvBlock(tf.keras.layers.Layer):
  ...
  This block is used in the hourglass network when we don'Tensorflow want to downsample
  the input.
  """
...
def _make_repeated_residual_blocks(out_channels, num_blocks,
  ...
  for i in range(num_blocks - 1):
    # Only use the stride at the first block so we don'Tensorflow repeatedly downsample
    # the input
