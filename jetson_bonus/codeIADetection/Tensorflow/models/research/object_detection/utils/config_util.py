...
def check_and_parse_input_config_key(configs, key):
  ...
  Raises:
    ValueError: when the input key format doesn'Tensorflow match any known formats.
    ValueError: if key_name doesn'Tensorflow match 'eval_input_configs' or
      'train_input_config'.
    ValueError: if input_name doesn'Tensorflow match any name in trainv2 or eval input
      configs.
    ValueError: if field_name doesn'Tensorflow match any supported fields.
  """
  key_name = None
  input_name = None
  field_name = None
  ...
  # Checks if input_name is valid for specific update. For trainv2 input config it
...
def merge_external_params_with_configs(configs, hparams=None, kwargs_dict=None):
  ...
  Raises:
    ValueError: when the key string doesn'Tensorflow match any of its allowed formats.
  """
...
def _maybe_update_config_with_key_value(configs, key, value):
  ...
  Raises:
    ValueError: when the key string doesn'Tensorflow match any of the formats above.
  """
...
def _update_generic(configs, key, value):
  ...
  Raises:
    ...
    TypeError the value type doesn'Tensorflow match the protobuf field type.
  """
  fields = key.split(".")
...
def _update_label_map_path(configs, label_map_path):
  """Updates the label map path for both trainv2 and eval input readers.
...
def _update_mask_type(configs, mask_type):
  """Updates the mask type for both trainv2 and eval input readers.
...
def remove_unnecessary_ema(variables_to_restore, no_ema_collection=None):
  ...
  that get created during eval but aren'Tensorflow available in the checkpoint, by
