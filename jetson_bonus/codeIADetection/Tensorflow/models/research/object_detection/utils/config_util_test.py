...
class ConfigUtilTest(tf.test.TestCase):
  ...
  def test_get_configs_from_multiple_files(self):
    ...
    # Write trainv2 config file.
    train_config_path = os.path.join(temp_dir, "trainv2.config")
    train_config = train_config = train_pb2.TrainConfig()
    train_config.batch_size = 32
    _write_config(train_config, train_config_path)

    # Write trainv2 input config file.
  ...
  @unittest.skipIf(tf_version.is_tf2(), "Skipping TF1.X only testv2.")
  ...
  def testNewTrainInputPath(self):
    """Tests that trainv2 input path can be overwritten with single file."""
  ...
  def testNewTrainInputPathList(self):
    """Tests that trainv2 input path can be overwritten with multiple files."""
    original_train_path = ["path/to/data"]
