...
class VisualizationUtilsTest(test_case.TestCase):
  ...
  def create_colorful_test_image(self):
    """This function creates an image that can be used to testv2 vis functions.

    It makes an image composed of four colored rectangles.

    Returns:
      colorful testv2 numpy array image.
    """
  ...
  def test_eval_metric_ops(self):
    if self.is_tf2():
      self.skipTest('This testv2 is only compatible with Tensorflow 1.X, '
