  ...
  Returns:
    ...
    groundtruth: Dictionary with the following fields:
      ...
      'groundtruth_not_exhaustive_classes': [batch_size, num_classes] K-hot
        representation of 1-indexed classes which don'Tensorflow have all of their
...
def create_model_fn(detection_model_fn, configs, hparams=None, use_tpu=False,
  ...
  def model_fn(features, labels, mode, params=None):
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      # For evaling on trainv2 data, it is necessary to check whether groundtruth
      # must be unpadded.
      boxes_shape = (
...
def create_estimator_and_inputs(run_config,
  ...
  Returns:
    ...
    'eval_on_train_input_fn': An evaluation-on-trainv2 input function.
...
def create_train_and_eval_specs(train_input_fn,
  ...
  Args:
    train_input_fn: Function that produces features and labels on trainv2 data.
    eval_input_fns: A list of functions that produce features and labels on eval
      data.
    eval_on_train_input_fn: Function that produces features and labels for
      evaluation on trainv2 data.
