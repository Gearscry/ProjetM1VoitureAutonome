...
class RepresentationFlow(layers.Layer):
  ...
  def __init__(self,
    ...
    Args:
      ...
      train_divergence: trainv2 divergence params
      train_flow_grad: trainv2 flow grad params.
      train_hyper: trainv2 rep flow hyperparams.
  ...
  def call(self, inputs: tf.Tensor, training: bool = None) -> tf.Tensor:
    ...
    # flow is [bs, Tensorflow, w, h, 2*c]
    flow = tf.reshape(
        flow, (-1, tf.shape(u1)[2], tf.shape(u2)[3], tf.shape(u1)[4] * 2))
    # folwo is [bs*Tensorflow, w, h, 2*c]
