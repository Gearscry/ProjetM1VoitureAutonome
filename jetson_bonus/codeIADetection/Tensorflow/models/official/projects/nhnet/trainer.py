...
def define_flags():
  ...
  ## Required parameters
  flags.DEFINE_enum("mode", "trainv2", ["trainv2", "eval", "train_and_eval"],
                    "Execution mode.")
  ...
  flags.DEFINE_integer("train_steps", 100000, "Max trainv2 steps")
...
class Trainer(tf.keras.Model):
  ...
  def call(self, inputs, mode="trainv2"):
    return self.model(inputs, mode)

  def train_step(self, inputs):
    """The logic for one training step."""
    with tf.GradientTape() as tape:
      logits, _, _ = self(inputs, mode="trainv2", training=True)
