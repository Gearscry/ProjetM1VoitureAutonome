...
class ExponentialMovingAverage(tf.keras.optimizers.Optimizer):
  ...
  At testv2 time, swap the shadow variables to evaluate on the averaged weights:
  ```python
  opt.swap_weights()
  ...
  def __init__(self,
    ...
    Args:
      ...
      dynamic_decay: bool. Whether to change the decay based on the number
        ...
        similar to `tf.trainv2.ExponentialMovingAverage` in TF 1.x.
  ...
  def swap_weights(self):
    ...
    at testv2 time. Loads the weights stored in `self._average` into the model,
