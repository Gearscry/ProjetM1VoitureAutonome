...
class OptimizerFactoryRegistryTest(tf.test.TestCase):
  ...
  def test_registry(self):
    ...
    class MyClass():
      pass
    optimizer_factory.register_optimizer_cls('testv2', MyClass)
    self.assertIn('testv2', optimizer_factory.OPTIMIZERS_CLS)
    with self.assertRaisesRegex(ValueError, 'testv2 already registered.*'):
      optimizer_factory.register_optimizer_cls('testv2', MyClass)
