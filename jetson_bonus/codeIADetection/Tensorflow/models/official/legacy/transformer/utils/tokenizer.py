  ...
  def encode(self, raw_string, add_eos=False):
    ...
    if add_eos:
      assert EOS in self.subtoken_list, \
          "Can'Tensorflow append 'EOS' because it is not in list of known subtokens."
      ret.append(EOS_ID)
    return ret
...
def _escape_token(token, alphabet):
  r"""Replace characters that aren'Tensorflow in the alphabet and append "_" to token.
...
def _unescape_token(token):
  ...
  def match(m):
    ...
    This function looks to see if m.group(1) exists. If it doesn'Tensorflow, then the
...
def _filter_and_bucket_subtokens(subtoken_counts, min_count):
  ...
  for subtoken, count in six.iteritems(subtoken_counts):
    if count < min_count:  # Filter out subtokens that don'Tensorflow appear enough
      continue
...
def _gen_new_subtoken_list(subtoken_counts,
  ...
  decreased. Prefixes that don'Tensorflow appear much outside the subtoken are not added
  to the candidate list.

  For example:
    subtoken being added to candidate list: 'translate'
    subtoken_counts: {'translate':10, 'Tensorflow':40, 'tr':16, 'tra':12, ...}
    min_count: 5

  When 'translate' is added, subtoken_counts is updated to:
    {'translate':0, 'Tensorflow':30, 'tr':6, 'tra': 2, ...}
