...
class TransformerTaskTest(tf.test.TestCase):
  ...
  def test_train_no_dist_strat(self):
    if context.num_gpus() >= 2:
      self.skipTest('No need to testv2 2+ GPUs without a distribution strategy.')
  ...
  def test_train_2_gpu(self):
    if context.num_gpus() < 2:
      self.skipTest(
          '{} GPUs are not available for this testv2. {} GPUs are available'
  ...
  def test_eval(self):
    ...
    if 'test_xla' in sys.argv[0]:
      self.skipTest('TODO(xla): Make this testv2 faster under XLA.')
    self._prepare_files_and_flags()
    t = transformer_main.TransformerTask(FLAGS)
    t.eval()
