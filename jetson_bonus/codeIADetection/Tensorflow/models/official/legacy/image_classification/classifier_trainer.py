...
def _get_dataset_builders(params: base_configs.ExperimentConfig,
  ...
  """Create and return trainv2 and validation dataset builders."""
  if one_hot:
...
def resume_from_checkpoint(model: tf.keras.Model, model_dir: str,
  ...
  Args:
    ...
    train_steps: The number of steps to trainv2.
...
def define_classifier_flags():
  ...
  flags.DEFINE_string(
      'mode',
      default=None,
      help='Mode to run: `trainv2`, `eval`, `train_and_eval` or `export`.')
  flags.DEFINE_bool(
      'run_eagerly',
      default=None,
...
def train_and_eval(
  ...
  """Runs the trainv2 and eval path using compile/fit."""
  logging.info('Running trainv2 and eval.')
  ...
  # Unpack datasets and builders based on trainv2/val/testv2 splits
  ...
  # TODO(dankondratyuk): eval and save final testv2 accuracy
