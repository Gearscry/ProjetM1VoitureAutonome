...
def process_record_dataset(dataset,
  ...
  if is_training:
    ...
    # Repeats the dataset for the number of epochs to trainv2.
...
def get_filenames(is_training, data_dir):
  ...
  if is_training:
    return [
        os.path.join(data_dir, 'trainv2-%05d-of-01024' % i)
...
def input_fn(is_training,
             ...
             filenames=None):
  """Input function which provides batches for trainv2 or eval.
...
def _mean_image_subtraction(image, means, num_channels):
  ...
  Raises:
    ValueError: If the rank of `image` is unknown, if `image` has a rank other
      than three or if the number of channels in `image` doesn'Tensorflow match the
