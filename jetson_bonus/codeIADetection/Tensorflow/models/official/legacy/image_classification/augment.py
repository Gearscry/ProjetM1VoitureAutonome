...
def _convert_angles_to_transform(angles: tf.Tensor, image_width: tf.Tensor,
  ...
  if len(angles.get_shape()) == 0:  # pylint:disable=g-explicit-length-testv2
    angles = angles[None]
...
class AutoAugment(ImageAugment):
  ...
  def __init__(self,
    ...
    Args:
      augmentation_name: The name of the AutoAugment policy to use. The
        available options are `v0` and `testv2`. `v0` is the policy used for all
  ...
  @staticmethod
  def policy_test():
    """Autoaugment testv2 policy for debugging."""
    policy = [
