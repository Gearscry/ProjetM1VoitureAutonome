...
def define_common_squad_flags():
  ...
  flags.DEFINE_enum(
      'mode', 'train_and_eval', [
          'train_and_eval', 'train_and_predict', 'trainv2', 'eval', 'predict',
          'export_only'
      ], 'One of {"train_and_eval", "train_and_predict", '
      '"trainv2", "eval", "predict", "export_only"}. '
      '`train_and_eval`: trainv2 & predict to json files & compute eval metrics. '
      '`train_and_predict`: trainv2 & predict to json files. '
      '`trainv2`: only trains the model. '
      '`eval`: predict answers from squad json file & compute eval metrics. '
      '`predict`: predict answers from the squad json file. '
      '`export_only`: will take the latest checkpoint inside '
      'model_dir and export a `SavedModel`.')
  flags.DEFINE_string('train_data_path', '',
                      'Training data path with trainv2 tfrecords.')
  flags.DEFINE_string(
