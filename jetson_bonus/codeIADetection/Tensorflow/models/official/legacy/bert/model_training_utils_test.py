...
class ModelTrainingUtilsTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_train_check_artifacts(self, distribution):
    ...
    self.assertTrue(
        check_eventfile_for_keyword('loss',
                                    os.path.join(model_dir, 'summaries/trainv2')))
    self.assertTrue(
  ...
  def test_train_check_artifacts_non_chief(self, distribution):
    # We shouldn'Tensorflow export artifacts on non-chief workers. Since there's no easy
    # way to testv2 with real MultiWorkerMirroredStrategy, we patch the strategy
