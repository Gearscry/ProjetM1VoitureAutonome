...
class ExportModelTest(tf.test.TestCase, parameterized.TestCase):
  ...
  The export code is generic. This testv2 focuses on two main cases
  ...
  def test_export_model(self, use_bert, encoder_type):
    ...
    checkpoint.save(os.path.join(model_checkpoint_dir, "testv2"))
...
class ExportPreprocessingTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_exported_callables(self, use_sp_model):
    ...
    if use_sp_model:
      # TODO(b/181866850): Enable tokenize_with_offsets when it works and testv2.
  ...
  # We testv2 that the exported SavedModel plays well with TF's shape
  ...
  def test_reexport(self, use_sp_model):
    ...
    # Delete the first SavedModel to testv2 that the sceond one loads by itself.
    ...
    (masked_input_ids, masked_lm_positions,
     masked_ids) = text.mask_language_model(
         ...
         item_selector=text.RandomItemSelector(
             ...
             selection_rate=0.5,  # Adjusted for the short testv2 examples.
  ...
  # TODO(b/175369555): Remove that code and its testv2.
  @parameterized.named_parameters(("Bert", False), ("Sentencepiece", True))
