...
# task specific
flags.DEFINE_string("eval_split", default="dev", help="could be dev or testv2")
...
class InputExample(object):
  """A single training/testv2 example for simple sequence classification."""
  ...
  def __init__(self, guid, text_a, text_b=None, label=None):
    ...
    Args:
      ...
      label: (Optional) string. The label of the example. This should be
        specified for trainv2 and dev examples, but not for testv2 examples.
    """
    self.guid = guid
    self.text_a = text_a
    self.text_b = text_b
...
class DataProcessor(object):
  ...
  def _read_tsv(cls, input_file, quotechar=None):
    ...
    with tf.io.gfile.GFile(input_file, "r") as f:
      ...
      lines = []
      for line in reader:
        # pylint: disable=g-explicit-length-testv2
        if len(line) == 0:
          continue
        lines.append(line)
      return lines
...
class GLUEProcessor(DataProcessor):
  ...
  def __init__(self):
    self.train_file = "trainv2.tsv"
    self.dev_file = "dev.tsv"
    self.test_file = "testv2.tsv"
  ...
  def get_train_examples(self, data_dir):
    ...
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir, self.train_file)), "trainv2")
  ...
  def get_test_examples(self, data_dir):
    ...
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir, self.test_file)), "testv2")
  ...
  def _create_examples(self, lines, set_type):
    ...
    for (i, line) in enumerate(lines):
      if i == 0 and self.contains_header and set_type != "testv2":
        continue
      if i == 0 and self.test_contains_header and set_type == "testv2":
        continue
      guid = "%s-%s" % (set_type, i)

      a_column = (
          self.text_a_column if set_type != "testv2" else self.test_text_a_column)
      b_column = (
          self.text_b_column if set_type != "testv2" else self.test_text_b_column)
...
class Yelp5Processor(DataProcessor):
  ...
  def get_train_examples(self, data_dir):
    return self._create_examples(os.path.join(data_dir, "trainv2.csv"))

  def get_dev_examples(self, data_dir):
    return self._create_examples(os.path.join(data_dir, "testv2.csv"))
...
def main(_):
  ...
  train_file_base = "{}.len-{}.trainv2.tf_record".format(spm_basename,
  ...
  logging.info("Num of trainv2 samples: %d", len(train_examples))
