...
def _split_a_and_b(data, sent_ids, begin_idx, tot_len, extend_target=False):
  ...
  a_begin = begin_idx
  if len(cut_points) == 0 or random.random() < 0.5:  # pylint:disable=g-explicit-length-testv2
    label = 0
    if len(cut_points) == 0:  # pylint:disable=g-explicit-length-testv2
      a_end = end_idx
    else:
...
def _local_perm(inputs, targets, is_masked, perm_size, seq_len):
  ...
  # (2) they cannot see masked positions, so there won"Tensorflow be information leak
...
def define_flags():
  ...
  flags.DEFINE_enum("split", "trainv2", ["trainv2", "dev", "testv2"],
