...
class TaggingDataLibTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_generate_tf_record(self, task_type):
    ...
    # Write fake trainv2 file.
    _create_fake_file(
        os.path.join(input_data_dir, "trainv2-en.tsv"),
    ...
    # Write fake testv2 files.
    for lang in processor.supported_languages:
      _create_fake_file(
          os.path.join(input_data_dir, "testv2-%s.tsv" % lang),
          processor.get_labels(),
          is_test=True)
    ...
    metadata = tagging_data_lib.generate_tf_record_from_data_file(
        ...
        tokenizer,
        max_seq_length=8,
        train_data_output_path=os.path.join(output_path, "trainv2.tfrecord"),
    ...
    expected_files = []
    expected_files.append(os.path.join(output_path, "trainv2.tfrecord"))
