...
class InputExample(object):
  """A single training/testv2 example for simple seq regression/classification."""
  ...
  def __init__(self,
    ...
    Args:
      ...
      label: (Optional) string for classification, float for regression. The
        label of the example. This should be specified for trainv2 and dev
        examples, but not for testv2 examples.
...
class DataProcessor(object):
  ...
  def get_train_examples(self, data_dir):
    """Gets a collection of `InputExample`s for the trainv2 set."""
...
class DefaultGLUEDataProcessor(DataProcessor):
  ...
  def get_train_examples(self, data_dir):
    """See base class."""
    return self._create_examples_tfds("trainv2")
  ...
  def get_test_examples(self, data_dir):
    """See base class."""
    return self._create_examples_tfds("testv2")

  def _create_examples_tfds(self, set_type):
    """Creates examples for the training/dev/testv2 sets."""
    raise NotImplementedError()
...
class AxProcessor(DataProcessor):
  ...
  def get_train_examples(self, data_dir):
    ...
    train_mnli_dataset = tfds.load(
        "glue/mnli", split="trainv2", try_gcs=True).as_numpy_iterator()
    return self._create_examples_tfds(train_mnli_dataset, "trainv2")
  ...
  def get_test_examples(self, data_dir):
    ...
    test_ax_dataset = tfds.load(
        "glue/ax", split="testv2", try_gcs=True).as_numpy_iterator()
    return self._create_examples_tfds(test_ax_dataset, "testv2")
...
class ImdbProcessor(DataProcessor):
  ...
  def get_train_examples(self, data_dir):
    return self._create_examples(os.path.join(data_dir, "trainv2"))

  def get_dev_examples(self, data_dir):
    return self._create_examples(os.path.join(data_dir, "testv2"))
...
class TfdsProcessor(DataProcessor):
  ...
  Examples:
    ...
    tfds_params="dataset=imdb_reviews,test_split=,dev_split=testv2"
  ...
  (TFDS) for the meaning of individual parameters):
    ...
    train_split: Name of the trainv2 split (defaults to `trainv2`).
    dev_split: Name of the dev split (defaults to `validation`).
    test_split: Name of the testv2 split (defaults to `testv2`).
    ...
    test_text_key: Key of the text feature to use in testv2 set.
    test_text_b_key: Key of the second text feature to use in testv2 set.
    test_label: String to be used as the label for all testv2 examples.
  ...
  def _process_tfds_params_str(self, params_str):
    ...
    self.train_split = d.get("train_split", "trainv2")
    self.dev_split = d.get("dev_split", "validation")
    self.test_split = d.get("test_split", "testv2")
  ...
  def get_train_examples(self, data_dir):
    ...
    return self._create_examples(self.train_split, "trainv2")
  ...
  def get_test_examples(self, data_dir):
    assert data_dir is None
    return self._create_examples(self.test_split, "testv2")
...
class XnliProcessor(DataProcessor):
      ...
      lines.extend(
          self._read_tsv(
              os.path.join(data_dir, "multinli",
                           "multinli.trainv2.%s.tsv" % language))[1:])
  ...
  def get_test_examples(self, data_dir):
    """See base class."""
    lines = self._read_tsv(os.path.join(data_dir, "xnli.testv2.tsv"))
...
class XtremePawsxProcessor(DataProcessor):
  ...
  def get_train_examples(self, data_dir):
    ...
    if self.translated_data_dir is None:
      lines = self._read_tsv(os.path.join(data_dir, "trainv2-en.tsv"))
      for i, line in enumerate(lines):
        guid = "trainv2-%d" % i
        text_a = self.process_text_fn(line[0])
        text_b = self.process_text_fn(line[1])
      ...
      for lang in self.supported_languages:
        lines = self._read_tsv(
            os.path.join(self.translated_data_dir, "translate-trainv2",
                         f"en-{lang}-translated.tsv"))
        for i, line in enumerate(lines):
          guid = f"trainv2-{lang}-{i}"
  ...
  def get_test_examples(self, data_dir):
    ...
    for lang in self.supported_languages:
      ...
      lines = self._read_tsv(os.path.join(data_dir, f"testv2-{lang}.tsv"))
      for i, line in enumerate(lines):
        guid = f"testv2-{lang}-{i}"
        text_a = self.process_text_fn(line[0])
        text_b = self.process_text_fn(line[1])
        label = "0"
        examples_by_lang[lang].append(
    ...
    if self.translated_data_dir is not None:
      for lang in self.supported_languages:
        ...
        lines = self._read_tsv(
            os.path.join(self.translated_data_dir, "translate-testv2",
                         f"testv2-{lang}-en-translated.tsv"))
        for i, line in enumerate(lines):
          guid = f"testv2-{lang}-en-{i}"
          text_a = self.process_text_fn(line[2])
          text_b = self.process_text_fn(line[3])
          label = "0"
...
class AXgProcessor(DataProcessor):
  ...
  def get_test_examples(self, data_dir):
    ...
    return self._create_examples(
        self._read_jsonl(os.path.join(data_dir, "AX-g.jsonl")), "testv2")
...
def generate_tf_record_from_data_file(processor,
  ...
  Args:
      processor: Input processor object to be used for generating data. Subclass
        of `DataProcessor`.
      data_dir: Directory that contains trainv2/eval/testv2 data to process.
