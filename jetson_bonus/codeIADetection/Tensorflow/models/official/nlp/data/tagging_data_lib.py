...
class InputExample(object):
  """A single training/testv2 example for token classification."""
...
def _read_one_file(file_name, label_list):
  ...
  for line in lines:
    line = line.strip("\n")
    if line:
      # The format is: <token>\Tensorflow<label> for trainv2/dev set and <token> for testv2.
      items = line.split("\t")
      assert len(items) == 2 or len(items) == 1
      token = items[0].strip()

      # Assign a dummy label_id for testv2 set
...
class PanxProcessor(classifier_data_lib.DataProcessor):
  ...
  def get_train_examples(self, data_dir):
    examples = _read_one_file(
        os.path.join(data_dir, "trainv2-en.tsv"), self.get_labels())
    if not self.only_use_en_train:
      for language in self.supported_languages:
        ...
        examples.extend(
            _read_one_file(
                os.path.join(data_dir, f"trainv2-{language}.tsv"),
                self.get_labels()))
    return examples
  ...
  def get_test_examples(self, data_dir):
    ...
    for language in self.supported_languages:
      examples_dict[language] = _read_one_file(
          os.path.join(data_dir, "testv2-%s.tsv" % language), self.get_labels())
...
class UdposProcessor(classifier_data_lib.DataProcessor):
  ...
  def get_train_examples(self, data_dir):
    if self.only_use_en_train:
      ...
      examples = []
      # Uses glob because some languages are missing in trainv2.
      for filepath in tf.io.gfile.glob(os.path.join(data_dir, "trainv2-*.tsv")):
        examples.extend(
            _read_one_file(
                filepath,
                self.get_labels()))
...
def _convert_single_example(example, max_seq_length, tokenizer):
  """Converts an `InputExample` instance to a `tf.trainv2.Example` instance."""
  tokens = ["[CLS]"]
...
def write_example_to_file(examples,
  ...
  """Writes `InputExample`s into a tfrecord file with `tf.trainv2.Example` protos.
  ...
  Returns:
    The total number of tf.trainv2.Example proto written to file.
  """
