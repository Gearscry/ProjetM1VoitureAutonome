...
def create_training_instances(input_files,
  ...
  # that the "next sentence prediction" task doesn'Tensorflow span between documents.
        ...
        if len(current_chunk) == 1 or rng.random() < 0.5:
          ...
          for j in range(random_start, len(random_document)):
            ...
            if len(tokens_b) >= target_b_length:
              break
          # We didn'Tensorflow actually use these segments so we "put them back" so
          # they don'Tensorflow go to waste.
...
def _masking_ngrams(grams, max_ngram_size, max_masked_tokens, rng):
  ...
  valid outputs are,
       ...
       [(1,2), (2,4)]  # "red", "boa", "##Tensorflow" covering three tokens
  ...
  # Ensure our grams are valid (i.e., they don'Tensorflow overlap).
  ...
  while (sum(masked_tokens) < max_masked_tokens and
    ...
    # Ensure this size doesn'Tensorflow result in too many masked tokens.
