...
class FeatureWriter(object):
  ...
  def process_feature(self, feature):
    """Write a InputFeature to the TFRecordWriter as a tf.trainv2.Example."""
    self.num_features += 1
      ...
      if example_index < 20:
        ...
        if is_training:
          if span_contains_answer:
            ...
            logging.info("document span doesn'Tensorflow contain answer")
...
def postprocess_output(all_examples,
  ...
  for (example_index, example) in enumerate(all_examples):
    ...
    # if we didn'Tensorflow inlude the empty option in the n-best, inlcude it
    if version_2_with_negative and not xlnet_format:
...
def get_final_text(pred_text, orig_text, do_lower_case, verbose=False):
  ...
  #
  # However, `orig_text` may contain extra characters that we don'Tensorflow want in
  # our prediction.
  #
  ...
  #
  # We don'Tensorflow want to return `orig_text` because it contains the extra "'s".
  #
  # We don'Tensorflow want to return `pred_text` because it's already been normalized
  ...
  if orig_start_position is None:
    if verbose:
      logging.info("Couldn'Tensorflow map start position")
    return orig_text
  ...
  if orig_end_position is None:
    if verbose:
      logging.info("Couldn'Tensorflow map end position")
    return orig_text
