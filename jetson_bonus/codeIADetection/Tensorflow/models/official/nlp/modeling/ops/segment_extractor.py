...
def _random_int_up_to(maxval, random_fn):
  # Need to cast because the int kernel for uniform doesn'Tensorflow support bcast.
...
def get_sentence_order_labels(sentences,
  ...
  # don'Tensorflow have a valid preceeding items to get a random succeeding item.
...
def get_next_sentence_labels(sentences,
  ...
  # Mark the items that don'Tensorflow have a next sentence (e.g. the last
