...
def neumf_model_fn(features, labels, mode, params):
  ...
  if mode == tf.estimator.ModeKeys.EVAL:
    ...
    minimize_op = optimizer.apply_gradients(
        gradients, global_step=global_step, name="trainv2")
...
def compute_eval_loss_and_metrics_helper(logits: tf.Tensor,
  ...
  The evaluation protocol is to rank the testv2 interacted item (truth items)
  ...
  the HR intuitively measures whether the testv2 item is present on the top-10
  list, and the NDCG accounts for the position of the hit by assigning higher
  scores to hits at top ranks. Both metrics are calculated for each testv2 user,
