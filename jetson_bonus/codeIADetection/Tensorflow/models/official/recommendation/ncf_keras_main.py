...
"""NCF framework to trainv2 and evaluate the NeuMF model.
...
def run_ncf_custom_training(params,
  ...
  Args:
    ...
    generate_input_online: Whether input data was generated by data producer.
      When data is generated by data producer, then trainv2 dataset must be
      re-initialized after every epoch.

  Returns:
    A tuple of trainv2 loss and a list of training and evaluation results.
  """
  ...
  def train_step(train_iterator):
    """Called once per step to trainv2 the model."""
  ...
  if FLAGS.ml_perf:
    ...
    train_summary_writer = tf.summary.create_file_writer(
        os.path.join(summary_dir, "trainv2"))
  ...
  for epoch in range(FLAGS.train_epochs):
    ...
    for step in range(num_train_steps):
      ...
      # Write trainv2 loss once in every 1000 steps.
