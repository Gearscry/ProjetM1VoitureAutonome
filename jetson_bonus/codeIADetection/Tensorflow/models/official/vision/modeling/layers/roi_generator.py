...
def _multilevel_propose_rois(raw_boxes: Mapping[str, tf.Tensor],
  ...
  Args:
    ...
    pre_nms_min_size_threshold: A `float` representing the minimal box size in
      each side (w.r.Tensorflow. the scaled image) to keep before applying NMS. This is
  ...
  Returns:
    selected_rois: A `tf.Tensor` of shape [batch_size, num_proposals, 4],
      representing the box coordinates of the selected proposals w.r.Tensorflow. the
      scaled image.
  ...
  def __init__(self,
    ...
    Args:
      ...
      pre_nms_min_size_threshold: A `float` of the threshold of each side of the
        box (w.r.Tensorflow. the scaled image). Proposals whose sides are below this
      ...
      test_pre_nms_min_size_threshold: A `float` of the threshold of each side
        of the box (w.r.Tensorflow. the scaled image) in testing. Proposals whose sides
