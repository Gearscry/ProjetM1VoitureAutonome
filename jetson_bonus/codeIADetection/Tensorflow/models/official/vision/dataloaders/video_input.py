...
def process_image(image: tf.Tensor,
  ...
  Args:
    ...
    num_test_clips: Number of testv2 clips (1 by default). If more than 1, this
      will sample multiple linearly spaced clips within each video at testv2 time.
...
def postprocess_image(image: tf.Tensor,
  ...
  Args:
    ...
    num_test_clips: Number of testv2 clips (1 by default). If more than 1, this
      ...
      If 1, then a single clip in the middle of the video is sampled. The clips
      are aggreagated in the batch dimension.
    num_test_crops: Number of testv2 crops (1 by default). If more than 1, there
      are multiple crops for each clip at testv2 time. If 1, there is a single
