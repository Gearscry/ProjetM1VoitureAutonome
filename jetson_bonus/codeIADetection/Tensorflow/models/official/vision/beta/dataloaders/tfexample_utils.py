...
# Lint as: python3
"""Utility functions to create tf.Example and tf.SequnceExample for testv2.

Example:video classification end-to-end testv2
i.e. from reading input file to trainv2 and eval.

```python
class FooTrainTest(tf.testv2.TestCase):

  def setUp(self):
    super(TrainTest, self).setUp()

    # Write the fake tf.trainv2.SequenceExample to file for testv2.
...
def create_detection_test_example(image_height: int, image_width: int,
  ...
  """Creates and returns a testv2 example containing box and mask annotations.

  Args:
    image_height: The height of testv2 image.
    image_width: The width of testv2 image.
    image_channel: The channel of testv2 image.
    num_instances: The number of object instances per image.

  Returns:
    A tf.trainv2.Example for testing.
  """
...
def create_segmentation_test_example(image_height: int, image_width: int,
                                     image_channel: int) -> tf.train.Example:
  """Creates and returns a testv2 example containing mask annotations.
